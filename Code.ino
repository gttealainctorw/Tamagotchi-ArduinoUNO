...
// Libraries for sleep mode and watchdog
#include <avr/sleep.h>
#include <avr/power.h>
#include <avr/wdt.h>

// Library for buzzer sounds
#include <CuteBuzzerSounds.h>

// Custom libraries for Alegotchi
...
/*
   Define display dimensions, used to create the display instance
   and reuse them throughout the code if needed
*/
...
void setup() {
  ...
  // Try to initialize the display at address 0x3C (yours may differ)
...
int var_type = 0; // Used to set the data type to retrieve (in this case int)
...
//____________________________ STATS
...
void displayEat(){
  ...
  display.fillRect(0, 16, 128, 48, SSD1306_BLACK); // Clear the screen
...
ISR (WDT_vect) {
    ...
    sec++; // sec = 450 --> one hour has passed
    Serial.print("sec: ");
    Serial.println(sec);
...
void turnOff() {
  ...
  // Every hour, if a stat is low, play a sound to attract attention
  // Only when the screen switch is off
